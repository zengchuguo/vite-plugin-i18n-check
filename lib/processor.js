  ...
  for (const attr of _scanTemplateAttrCandidates(cleanedTemplate)) {
    ...
    if (!attr.isDynamic) {
      ...
      for (const str of scanJsStrings(attr.value)) {
        ...
        // 兜底：子串（保持向后兼容）
        const rawStr = templateRaw.slice(absStart, absEnd)
        let newRawStr = rawStr
        let anyReplaced = false
        for (const m of str.content.matchAll(chineseRe())) {
          const key = reverseMap[m[0]]
          if (key) {
            const updated = replaceStringLiteral(newRawStr, m[0], key, '$t')
            if (updated !== newRawStr) { newRawStr = updated; anyReplaced = true }
          } else {
            untranslated.push({ file: relFile, line: strLine, text: m[0] })
          }
        }
        if (anyReplaced) allReplacements.push({ start: absStart, end: absEnd, replacement: newRawStr })
      }
    }
  }
